        ...
        <div class="container read-container"  >
           	...
           	<li><a class="button right-board" href="/board/qna/list">목록 보기</a></li>
				<c:if test="${not empty USER}">					
					<li style="margin-right: 10px"><a class="button right-board" href="/board/qna/write">글쓰기</a></li>	
				</c:if>            		            	            	
           	</ul>
             	...
             	<div class="board-view-header" style="margin-top: 65px">
		            <div class="sub_wrap" style="margin-top: 0px">
		            	 <p>
		      			...
		      			<form method="post" action="/board/qna/boardEditDirect"  id="directForm"> 
...
//////////////////////////////
		...
		$('.editBoardLoginedUser').click(function(){
			var replyCount=$("#boardComment2").text();
			var sId ='${sessionScope.USER.userId}';
			var bId='${boardVO.userId}';
			var isAdmin='${isAdmin}';
			if(parseInt(replyCount)>0 && isAdmin=="false"){
				alert("답변 달린 게시글은 수정할 수 없습니다.");
				return;
			}
			...
			if(type=='1'){				
				$.ajax({
					...
					success:function(result){	
						...
						if(re==1){
							$("#directForm").attr("action", "/board/qna/boardEdit");
		...
		$.ajax({
			...
			success:function(result){
				if(result=="success"){
					replyList(1);
					alert("등록 되었습니다.");
					$("#boardCommentForm").css("visibility", "hidden");
					$("#boardComment").css("height", "70px");	 
					$(e).text("답변 쓰기");
...
function replyList(page){
	...
	$.get("/board/replies/all/"+bno +"?page="+page+"&boardType=board_qna", function(data){
		//console.log(data);
...
function replyShow(e){
	$(".note-editable").html("");
	 ...
	 if($(e).text()=="답변 쓰기"){
		 $("#boardCommentForm").css("visibility", "visible");
		 $("#boardComment").css("height", "auto");	 
		 $(e).text("답변 닫기");
		 return;
	 }
