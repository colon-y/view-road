...
function listNewMemberRegistPerDay(currentMonth) {
	...
	$('.card-title').html("방문자수:&nbsp;  "+ym[0] + '년 ' + ym[1]+"월");
	//thisMonth = new Date(ym[0],  parseInt(ym[1]) - 1, 1);	
	$(".year-month").html(ym[0] + '년 ' + ym[1]+"월");
	
	$.ajax({
		url: "/admin/statistics/listVisitorStatistics",
		type:"POST",
		data: { currentMonth },
		success: function(response) {
			...
			$("#ipBarChartDiv").append('<canvas id="ipBarChart"></canvas>');
			$("#ipLineChartDiv").append('<canvas id="ipLineChart"></canvas>');
		  ...
		  if ($("#lineChart").length) {
		    ...
		    lineChart = new Chart(lineChartCanvas, {
	 		...
	 		if ($("#ipBarChart").length) {
				let ipBarChartCanvas = $("#ipBarChart").get(0).getContext("2d");
				let ipBarChart = new Chart(ipBarChartCanvas, {
					type: 'bar',
					data: ipMyChartData,
					options
				});
			...
			if ($("#ipLineChart").length) {
				let ipLinChartCanvas = $("#ipLineChart").get(0).getContext("2d");
				let ipLineChart = new Chart(ipLinChartCanvas, {
					type: 'line',
					data: ipMyChartData,
					options
				});
...
function calendarInit() {
    ...
    function renderCalender(thisMonth) {
		...
		$(".card-title").html("방문자수: &nbsp;"+currentYear + '년 ' + curMonth+"월");
