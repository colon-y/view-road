                    ...
                    <div class="row">
                        ...
                        <div class="col-xl-12 col-lg-12">
                 		...
                 		<div class="mb-5" id="note-head" >
                 			...
                 			<div>
                 				<button class="btn btn-default  mr-3"  onclick="location.href='/members/note/receiveList'">개별 쪽지함</button>
                 				<button class="btn btn-info">유저별 쪽지함</button>
                 			</div>
                 		</div>                   
                <div class="col-xl-6 col-lg-6">
                 
               <form action="/members/note/groupMessgeList" id="searchForm">
                        <div class="mb-3" >
                          <div class="row">
           		...
           		<strong>상세보기를 할경우 전체 읽음으로 처리됩니다.</strong>
				...
				<div class="table-responsive">
                                <table class="table table-bordered table-hover" id="dataTable" >
                                    ...
                                    <tbody>
                                    <c:if test="${empty groupMessgeList }">
                                           <tr>
                                    ...
                                    <c:forEach items="${groupMessgeList}" var="row">
                                        	<tr>
                                              <td class="text-center align-middle" style="height: 70px;vertical-align: inherit;">                                                                                     	
                                            	...
                                            	<p class="text-center"><small>[${row.sendName}]</small></p>
                                             </td>
                                        
                                            <td onclick="location.href='/members/note/read/${row.noteId}?sendId=${row.sendId}&group=1'" class="align-middle">  
	                                            <div class="td-title">                                                                                   	
                                            ...
                                            <td class="text-center align-middle" >
                                             <c:choose>
                                             	<c:when test="${row.readCnt ==0}">                                             	  
                                             		...
                                             		onclick="location.href='/members/note/read/${row.noteId}?sendId=${row.sendId}'">
                                            ...
                                            <td class="align-middle">
                                            	<div class="delete">
                                            		...
                                            		<button class="btn btn-danger" onclick="deleteGroupNote('${row.sendId}', '${row.sendName}')">삭제</button>
                                            	</div>                                            
                                            </td>                                        
                                        </tr>
                                     </c:forEach>
...
<script>
function deleteGroupNote(sendId, sendName){	
	if(confirm("정말 " +sendName+"과 쪽지 내용을 전부 삭제 하시겠습니까?")) {
		$.ajax({
			url:"/members/note/deleteGroupNote",
			type:"delete",
			data:{sendId},
			success:function(res){
				if(res=="success"){
					location.href="/members/note/groupMessgeList";
				}
			},
...
function deleteSelectGroup(){
  let chk_val=[];
  $("input[type=checkbox][name=sendId]:checked").each(function(i, ival){
  ...
  if(chk_val.length===0){
    ...
    console.log(chk_val);
  if(confirm("정말 선택하신 유저와 쪽지 내용을 전부 삭제 하시겠습니까?")){
	 	jQuery.ajaxSettings.traditional = true;
		$.ajax({
			url:"/members/note/deleteSelectGroup",
			type:"delete",
			data:{sendId:chk_val},
			success:function(res){
				console.log(res);
